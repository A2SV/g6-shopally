...
func SetupRouter(cfg *config.Config, limiter *middleware.RateLimiter, searchHandler *handler.SearchHandler, alertHandler *handler.AlertHandler, priceHandler *handler.PriceHandler) *gin.Engine {
	router := gin.Default()
