...
import {
  ...
  VALIDATION_MESSAGES,
} from "@/utils/Profile/profileConstants";
...
export default function ProfileContent() {
  ...
  // Initialize data from API when available
  useEffect(() => {
    if (user) {
      setUserData({
  ...
  useEffect(() => {
    if (preferences) {
      ...
      setRegion(preferences.shippingRegion);
      setLanguage(preferences.languagePreference);
    }
  }, [preferences]);
  ...
  const handleImageUpload = async (
    ...
    if (file) {
      ...
      if (file.size > PROFILE_IMAGE_MAX_SIZE) {
        setShowMessage({
          type: "error",
          message: VALIDATION_MESSAGES.IMAGE_TOO_LARGE,
        });
        return;
      }
        ...
        <div className="space-y-6 sm:space-y-8">
          ...
          >
            <div className="flex items-center justify-center py-12">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[var(--color-brand-yellow)]"></div>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            {!isEditing ? (
              ...
              <button
                ...
                className="px-3 py-1 bg-gray-500 text-white font-medium rounded-md hover:bg-gray-600 transition-colors text-sm"
              >
                {t("Cancel")}
              </button>
            )}
                  ...
                  <button
                    ...
                    className={`px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 border ${
                      selectedCategories.includes(category.value)
                        ? "bg-[var(--color-brand-yellow)] text-gray-900 border-[var(--color-brand-yellow)]"
                        : isDarkMode
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    className={`w-full px-3 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--color-brand-yellow)] focus:border-transparent transition-colors text-sm ${
                      isDarkMode
                        ? "border-gray-600 bg-[var(--color-brand-white)] text-gray-900"
